import random
import time
#User experience perspective, put some sleeping time between computer's hands
"""new idea
User -We don't need to check whethere there is A or not
Just calculate as a 1 first then, create a new variable to count how many a is there 
when we press n to stop getting card. Put condition that if add 10 would exceed 21 or not. if it is then don't add. else: add
YAY
Computer- use user_qualified boolean value to distinguish difference between user_total and comp_total
"""
...
def add_a(a_count, total):
    global user_qualified
    ...
    while a_count:
        if not user_qualified:
            ...
            if int(total) + 10 >= 16 and int(total) + 10 < 22:
                total += 10
        a_count -= 1
...
def game_over():
    ...
    if play == "n":
        running = False
        print("\n\nThank you for playing my BLACKJACK")
...
while running:
    ...
    while True:   #if under 22, not 21 -> just true
        ...
        if response == "y":
            ...
            if a_count:
                user_total = add_a(a_count, user_total)
    ...
    if user_qualified:
        ...
        if a_count:
            comp_total = add_a(a_count, comp_total)
